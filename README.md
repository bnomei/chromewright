...
- `navigate` and `new_tab` reject unsafe schemes such as `data:` and `file:` unless the caller passes `allow_unsafe = true` on that specific request. `go_back` and `go_forward` apply the same gate: if a history move lands on a non-http(s) destination, the move is reverted and the call is rejected unless that request also sets `allow_unsafe = true`.
